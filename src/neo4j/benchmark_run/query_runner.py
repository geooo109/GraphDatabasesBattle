...
#TODO
    ...
    def mib_1(self, personId, minMessageLength):
        query = "MATCH (person:Person {id:" + personId + "})-[:KNOWS*1..2]-(chain_friend:Person)-[like:LIKES]->(comment:Comment) WHERE NOT (person = chain_friend) and comment.length >  " + minMessageLength + " WITH DISTINCT chain_friend, comment RETURN chain_friend.id as friendId, count (comment.id) as msgLikes ORDER BY friendId LIMIT 50"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result
    
    def mib_2(self, personId, tagClassInput, languageInput):
        query = "MATCH (person:Person {id:" + personId + "})-[:KNOWS]-(friend:Person)<-[:HAS_CREATOR]-(post:Post)-[:HAS_TAG]->(tag:Tag)-[:HAS_TYPE]->(tagClass:TagClass) WHERE (post.language = " + languageInput + ") AND (tagClass.name = " + tagClassInput + ") RETURN DISTINCT post.id as postId, tagClass.name as tagClassName"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result    
   
    def mib_3(self, personId, browserName):
        query = "MATCH (:Person {id:" + personId + "})-[:KNOWS*1..2]-(friend:Person)<-[:HAS_CREATOR]-(post:Post {browserUsed:" + browserName + "}) RETURN DISTINCT  post.id as finalId, post.creationDate as creationDate ORDER BY post.creationDate ASC LIMIT 10"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result    
    
    def mib_4(self, personId, dateInput):
        dateInput = self.getDate(dateInput)
        query = "MATCH (person:Person {id:" + personId + "}) <- [:HAS_MEMBER] - (forum:Forum) - [:HAS_TAG] -> (tag:Tag) <- [:HAS_TAG] - (post:Post) WHERE post.creationDate  < datetime(" + dateInput + ") RETURN DISTINCT post.id, post.content, post.creationDate ORDER BY post.creationDate ASC LIMIT 30"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result   

    def mib_5(self, personId):
        query = "MATCH (:Person {id:" + personId + "}) - [:WORK_AT] -> (:Company) - [:IS_LOCATED_IN] -> (country:Country) - [:IS_PART_OF] -> (continent:Continent) RETURN continent"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result       

    def mib_6(self, personId, dateInput, tagNameInput):
        dateInput = self.getDate(dateInput)
        query = "MATCH (person:Person {id:" + personId + "})-[:KNOWS*1..2]-(chain_friend:Person)- [:HAS_INTEREST] -> (tag:Tag {name:" + tagNameInput + "}) <- [:HAS_TAG] - (comment:Comment) WHERE comment.creationDate > datetime(" + dateInput + ") RETURN comment.id as commentId, comment.browserUsed as Browser, comment.creationDate as creationDate ORDER BY comment.creationDate ASC LIMIT 50"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result           
    
    def mib_7(self, personId, fristNameInput):
        query = "MATCH (:Person {id:" + personId + "})-[path:KNOWS*1..4]-(friend:Person)  WHERE friend.firstName = " + fristNameInput + " WITH friend, min(size(path)) AS distance ORDER BY distance ASC, friend.lastName ASC, friend.id DESC LIMIT 15 RETURN distance, friend.lastName, friend.id"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result

    def mib_8(self, personId, creationDateInput):
        creationDateInput = self.getDate(creationDateInput)
        query = "MATCH (person:Person {id:" + personId + "}) - [:IS_LOCATED_IN] -> (:City) - [:IS_PART_OF] -> (country:Country) <- [:IS_LOCATED_IN] - (message:Message) WHERE message.creationDate < datetime(" + creationDateInput + ") RETURN DISTINCT country.name, message.browserUsed"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result    
    
    def mib_9(self, personId, inputDate):
        inputDate = self.getDate(inputDate)
        query = "MATCH (person:Person {id:" + personId + "})-[:KNOWS*1..2]-(chain_friend:Person) WHERE person.id <> chain_friend.id WITH DISTINCT chain_friend as ff MATCH(ff:Person)<-[HAS_MEMBER]-(forum:Forum) WHERE forum.creationDate < datetime(" + inputDate + ") RETURN count(*) as counting"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result

    def mib_10(self, messageIdInput):
        query = "MATCH (m:Message {id : " + messageIdInput  + "})<-[:REPLY_OF]-(c:Comment)-[:HAS_CREATOR]->(p:Person)- [:STUDY_AT] -> (uni:University) RETURN  CASE uni WHEN null THEN \"\" ELSE uni.name END as UniversityName"
        result = self.session.run(query)
        #for records in result:
        #    print(records)
        return result
